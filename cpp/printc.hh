...
protected:
    //added by Tej
    static OpToken copy;
    static OpToken branch;
    static OpToken cbranch;
    static OpToken branchind;
    static OpToken call;
    static OpToken callind;
    static OpToken callother;
    static OpToken ret;
...
public:
    ...
    virtual void opCopy(const PcodeOp *op) { opUnary(&copy, op); }
    virtual void opLoad(const PcodeOp *op);
    virtual void opStore(const PcodeOp *op);
    virtual void opBranch(const PcodeOp *op) { opUnary(&branch, op); }
    virtual void opCbranch(const PcodeOp *op) { opUnary(&cbranch, op); }
    virtual void opBranchind(const PcodeOp *op) { opUnary(&branchind, op); }
    virtual void opCall(const PcodeOp *op) { opUnary(&call, op); }
    virtual void opCallind(const PcodeOp *op) { opUnary(&callind, op); }
    virtual void opCallother(const PcodeOp *op) { opUnary(&callother, op); }
    virtual void opConstructor(const PcodeOp *op,bool withNew);
    virtual void opReturn(const PcodeOp *op) { opUnary(&ret, op); }
